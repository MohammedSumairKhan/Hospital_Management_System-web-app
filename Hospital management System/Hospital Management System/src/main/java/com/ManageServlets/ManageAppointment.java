...
public class ManageAppointment extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			PreparedStatement pstmt=con.prepareStatement("delete from appointments where appointment_id = ? and appointment_date = ?");
			pstmt.setInt(1, id);
			pstmt.setString(2, password);
			int res=pstmt.executeUpdate();
			if(res==1) {
				resp.getWriter().println("<h1>appointment "+id+" deleted!!!<h1>");
			}
			else {
